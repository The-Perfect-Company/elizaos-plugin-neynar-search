...
export function loadCachedResults(): CacheEntry | null {
  try {
    ...
    if (!fs.existsSync(cachePath)) {
      elizaLogger.log("[NEYNAR-CACHE] No cache file found");
      return null;
    }
    ...
    if (entry.version !== CACHE_VERSION) {
      elizaLogger.log("[NEYNAR-CACHE] Cache version mismatch — invalidating");
    ...
    if (age > entry.ttlMs) {
      elizaLogger.log(`[NEYNAR-CACHE] Cache expired (age=${Math.round(age/1000/60)}m, ttl=${Math.round(entry.ttlMs/1000/60)}m)`);
      fs.unlinkSync(cachePath);
      return null;
    }

    elizaLogger.log(`[NEYNAR-CACHE] Cache HIT — age=${Math.round(age/1000/60)}m, ${entry.results.length} casts, ${entry.keywords.length} keywords`);
    return entry;
  } catch (err) {
    elizaLogger.warn("[NEYNAR-CACHE] Error loading cache: " + String(err));
    return null;
  }
}
...
export function saveCachedResults(
  ...
  try {
    ...
    elizaLogger.log(`[NEYNAR-CACHE] Cache SAVED — ${results.length} casts, expires in ${Math.round(CACHE_TTL_MS/1000/60)}m`);
  } catch (err) {
    elizaLogger.warn("[NEYNAR-CACHE] Error saving cache: " + String(err));
  }
}
...
export function clearCache(): void {
  try {
    ...
    if (fs.existsSync(cachePath)) {
      fs.unlinkSync(cachePath);
      elizaLogger.log("[NEYNAR-CACHE] Cache cleared");
    }
  } catch (err) {
    elizaLogger.warn("[NEYNAR-CACHE] Error clearing cache: " + String(err));
  }
}
